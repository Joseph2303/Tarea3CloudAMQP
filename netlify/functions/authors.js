...
exports.handler = async (event) => {
  ...
  try {
    ...
    try {
      ...
      console.error('[authors] JSON parse error:', e.message, 'raw:', event.body);
      return { statusCode: 400, headers, body: JSON.stringify({ error: 'JSON inválido' }) };
    }

    // Mapeo de acciones para diferido
    ...
    // Publicar operación en la cola (diferido)
    try {
      ...
      console.error('[authors] publishMessage error:', e);
      return {
        statusCode: 502,
        headers,
